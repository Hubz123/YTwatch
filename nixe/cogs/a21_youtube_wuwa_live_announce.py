...
class YouTubeWuWaLiveAnnouncer(commands.Cog):
    ...
    def _reload_watchlist(self):
        ...
        # - Some deployments store the watchlist file as a *list* of target objects.
        # - Newer deployments store a dict with metadata + "targets".
        raw = _read_json_any(WATCHLIST_PATH)
        if isinstance(raw, list):
            cfg: Dict[str, Any] = {"targets": raw}
            cfg_meta: Dict[str, Any] = {}
            tlist_raw: List[Dict[str, Any]] = raw
        elif isinstance(raw, dict):
            cfg = raw
            cfg_meta = cfg
            tlist_raw = (cfg.get("targets") or cfg.get("watchlist") or [])
        else:
            cfg = {}
            cfg_meta = {}
            tlist_raw = []
        ...
        rx_str = (ENV_REGEX_OVERRIDE or cfg_meta.get("title_whitelist_regex") or DEFAULT_TITLE_REGEX).strip()
        tpl = (ENV_TEMPLATE_OVERRIDE or cfg_meta.get("message_template") or DEFAULT_MESSAGE_TEMPLATE)
        ...
        for t in tlist:
            ...
            url = str(t.get("url") or t.get("channel_url") or t.get("channelUrl") or t.get("channelURL") or "").strip()
        ...
        # Emit watchlist visibility only when the watchlist *changes* (avoid log spam every poll).
        sig = tuple((t.url, t.channel_id, t.handle, t.query) for t in self.targets)
        if sig != getattr(self, "_watchlist_sig", None):
            self._watchlist_sig = sig
            if self.targets:
                first = self.targets[0].url or self.targets[0].channel_id or self.targets[0].query
                log.info("[yt-wuwa] watchlist loaded targets=%d first=%s", len(self.targets), first)
            else:
                log.warning("[yt-wuwa] watchlist loaded targets=0 (check %s)", WATCHLIST_PATH)
    ...
    async def _http_get_text(self, url: str) -> Optional[str]:
        """Fetch text with basic anti-cache hardening for YouTube RSS.

        YouTube RSS can be edge-cached; add cache-buster + no-cache headers so
        new uploads propagate faster (UI sometimes shows newer than RSS).
        """
        await self._ensure_session()
        ...
        if "www.youtube.com/feeds/videos.xml?channel_id=" in url:
            # Cache-buster + no-cache headers (best-effort; safe if ignored)
    ...
    async def _check_upload(self, t: Target) -> Optional[Tuple[Target, str, str, Optional[datetime], str]]:
        """
        Returns (target, video_id, title, published_ts_utc, creator_name) if newest upload matches whitelist.
        - Uses YouTube channel RSS: https://www.youtube.com/feeds/videos.xml?channel_id=UC...
        - Does NOT announce old uploads after restart when ONLY_NEW_AFTER_BOOT=1.
        """
        t = await self._resolve_channel(t)
        if not t.channel_id:
            return None

        rss_url = f"https://www.youtube.com/feeds/videos.xml?channel_id={t.channel_id}"
        xml_txt = await self._http_get_text(rss_url)
        if not xml_txt:
            return None
        ...
        # RSS is Atom. Namespace handling:
        ns = {
        ...
        vid = _find_text("yt:videoId")
        title = _find_text("atom:title")
        published_s = _find_text("atom:published")
        author_name = ""
        try:
            auth = entry.find("atom:author", ns)
            if auth is not None:
                nm = auth.find("atom:name", ns)
                author_name = (nm.text or "").strip() if nm is not None else ""
        except Exception:
            author_name = ""

        published_ts = _parse_iso_utc(published_s)
        ...
        # Prefer creator display name
        creator_name = (t.name or "").strip()
        if (not creator_name) or creator_name.startswith(("@", "＠")) or _UC_ID_LIKE_RE.match(creator_name):
            creator_name = (author_name or "").strip()
        if (not creator_name) or creator_name.startswith(("@", "＠")) or _UC_ID_LIKE_RE.match(creator_name):
            # last resort: resolve from channel page/oembed (best-effort, quick)
            try:
                base = t.base_url() or f"https://www.youtube.com/channel/{t.channel_id}"
                nm = await asyncio.wait_for(self._try_fetch_channel_name_oembed(base), timeout=3.0)
                if nm:
                    creator_name = nm
            except Exception:
                pass
        ...
        return t, vid, title, published_ts, creator_name
    ...
    async def loop(self):
        ...
        async def _run_one(tt):
            ...
            try:
                ...
                out = []
                # Check live first (fast path), but still check uploads so we can notify both.
                try:
                    r_live = await asyncio.wait_for(self._check_live(tt), timeout=timeout)
                    if r_live:
        ...
        # Guard: asyncio.wait() raises ValueError on empty set
        # This can happen if the watchlist/targets is empty (e.g., misconfig or file missing).
        if not tasks_list:
            if not getattr(self, '_warned_no_targets', False):
                self._warned_no_targets = True
                log.warning('[yt-wuwa] no targets configured; watcher idle (check data/youtube_wuwa_watchlist.json)')
            return
        ...
        for res in results:
            ...
            # Do not announce old items after this bot instance booted.
            # Requirement: after restart, do NOT re-announce old live or old uploads.
            if ONLY_NEW_AFTER_BOOT:
                if start_ts is None:
                    # Missing timestamp: treat as 'new enough' to allow a single announce.
                    start_ts = now
                    if kind == "live":
                ...
                if start_ts < (self.boot_time - timedelta(seconds=max(0, BOOT_GRACE_SECONDS))):
                    ...
                    if kind == "live":
                        ...
                        log.info("[yt-wuwa] suppress old-upload after boot: %s vid=%s age_min=%s", t.name, vid, age_min)
                    continue

            # Optional: suppress "too old" items even without restarts (0 disables)
            if ANNOUNCE_MAX_AGE_MINUTES > 0 and start_ts is not None:
                if (now - start_ts).total_seconds() > (ANNOUNCE_MAX_AGE_MINUTES * 60):
                    ...
                    if kind == "live":
                        ...
                        log.info("[yt-wuwa] suppress stale-upload: %s vid=%s age_min=%s", t.name, vid, age_min)
                    continue
            ...
            try:
                ...
                if delay_min is None:
                    log.info("[yt-wuwa] announced %s: %s vid=%s", kind, t.name, vid)
                else:
                    log.info("[yt-wuwa] announced %s: %s vid=%s delay_min=%s", kind, t.name, vid, delay_min)
            except Exception as e:
